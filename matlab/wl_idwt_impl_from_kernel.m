function x=wl_idwt_impl_reuse_kernel(x, rekernel, verbose)
    ...
    if verbose
        ...
        if rekernel.forward ~=  0
            disp('Error: wl_dwt_impl_from_kernel: Not a backward kernel.')    
        end 
    ...
    if rekernel.dims == 1
        ...
        if rekernel.transpose % if transpose, then f will we a dwt_kernel, 
    ...
    if rekernel.dims == 2
        ...
        if rekernel.transpose % if transpose, then f will we a dwt_kernel, 
            x =  wl_dwt2_impl_internal(x, rekernel.fx, rekernel.fy, ...
                    rekernel.prefilterx, rekernel.prefiltery, offsets,...
                    m, bd_mode, data_layout);
        else
    ...
    if rekernel.dims == 3 % if not give error message
        ...
        if rekernel.transpose % if transpose, then f will we a dwt_kernel, 
            x =  wl_dwt3_impl_internal(x, rekernel.fx, rekernel.fy, ...
                    rekernel.fz, rekernel.prefilterx, rekernel.prefiltery, ...
                    rekernel.prefilterz, offsets, m, bd_mode, data_layout);     
        else
